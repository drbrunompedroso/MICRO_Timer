...
{
    if (INTCONbits.TMR0IF == 1)  // a cada 25mseg
    {
        timer_500ms++;
        if (timer_500ms >= 20)   //20 * 25mseg
        {
            timer_500ms = 0;
...
void main(void) 
{
    ...
    lcd_write(1,1, "REAL CLOCK 8bits");
        
    while(1)
    {
        sprintf(bufferLCD, "%02d:%02d:%02d", counterHor, counterMin, counterSeg);       
