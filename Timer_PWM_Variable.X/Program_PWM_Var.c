...
void config_TIMER0()
{
    ...
    T0CON = 0XC0;       /* TMR0ON = 1       - Habilita TIMER0                       
                         * T08BIT = 1       - Modo de operação em 8 bits
                         * T0CS = 0         - Incremento pelo ciclo de máquina
                         * T0SE = 0         - Nào de aplica
                         * PSA = 0          - Prescaler habilitado
                         * T0PS<2:0>        - Prescaler de 1:2
                         */
    
    TMR0L = 256;          /* Valor inicial para o registrador TMR0
                         * 
                         * Cálculo do TIMER 8bits
                         * tempo desejado = 256 * prescaler * ciclo de máquina = 256 * 2 * [1/(FOSC/4)] = 256uSeg
                         * 
                         * 1mseg / 256useg = 3.9 repeticoes (4 repetições)
                         * 
                         * TMR0 = 256 - (tempo desejado / (ciclo de maquina * prescaler))
                         * TMR0 = 256 - (256useg / (0.5useg * 2)
                         * TMR0 = 256             
                         */
}
...
void __interrupt() intern_TMR0(void)
{
    if (INTCONbits.TMR0IF == 1)
    {
       ...
       if(duty_cicle <= 2)
        ...
        else if(duty_cicle > 2 && duty_cicle <=4)
        {        
           PORTBbits.RB0 = 0; 
